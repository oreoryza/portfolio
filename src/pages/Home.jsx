      ...
      <AnimatePresence initial={false}>
        {!door && (
          ...
          >
            ...
            >
              <p className={`font-gabarito rotate-180 border-t ${theme==="dark" ? "border-black" : "border-white"} w-full flex items-center py-4`} style={{ writingMode: 'vertical-rl' }}>{desc}</p>
              <div
                onClick={() => dispatch(doorToggle())}
                className="w-6 h-6 bg-yellow border-l-2 border-b-2 border-blue my-4 hover:border-l-4 hover:border-b-4 cursor-pointer"
              ></div>
            </div>
              <p className={`${theme==="dark" ? "" : "text-white"} absolute bottom-4 left-16 text-sm opacity-50 duration-300`}><i className="bi bi-c-circle"></i> ORYZA SATIVA</p>
